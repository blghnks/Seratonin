...
public class QueueDatabase extends SQLiteOpenHelper {
     ...
     */
    public void moveInQueue(int fromPosition, int toPosition) {
        ...
        try {
            ...
            db.execSQL("UPDATE " + TABLE_QUEUE + " SET " + COLUMN_POSITION + " = -1 WHERE " + COLUMN_POSITION + " = ?",
                    new Object[]{fromPosition});
            ...
            db.execSQL("UPDATE " + TABLE_QUEUE + " SET " + COLUMN_POSITION + " = ? WHERE " + COLUMN_POSITION + " = -1",
                    new Object[]{toPosition});
     ...
     */
    public void addToQueue(MusicFiles song) {
        SQLiteDatabase db = getWritableDatabase();
        
        // Get current max position
        int maxPosition = -1;
        Cursor cursor = db.rawQuery("SELECT MAX(" + COLUMN_POSITION + ") FROM " + TABLE_QUEUE, null);
    ...
    /**
     * Add a song to play next (after current position).
     */
    public void addToPlayNext(MusicFiles song, int currentIndex) {
        SQLiteDatabase db = getWritableDatabase();
        db.beginTransaction();
        try {
            int insertPosition = currentIndex + 1;
            
            // Shift all songs after insert position
            db.execSQL("UPDATE " + TABLE_QUEUE + " SET " + COLUMN_POSITION + " = " + COLUMN_POSITION + " + 1 " +
                    "WHERE " + COLUMN_POSITION + " >= ?", new Object[]{insertPosition});
            
            // Insert the new song
            ContentValues values = new ContentValues();
            values.put(COLUMN_POSITION, insertPosition);
            values.put(COLUMN_PATH, song.getPath());
            values.put(COLUMN_TITLE, song.getTitle());
            values.put(COLUMN_ARTIST, song.getArtist());
            values.put(COLUMN_ALBUM, song.getAlbum());
    ...
    public int getQueueSize() {
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT COUNT(*) FROM " + TABLE_QUEUE, null);
        int size = 0;
        if (cursor.moveToFirst()) {
    ...
    /**
     * Add multiple songs to the queue (for import).
     */
    public void addAllToQueue(ArrayList<MusicFiles> songs) {
        ...
        try {
            ...
            for (MusicFiles song : songs) {
                maxPosition++;
                ContentValues values = new ContentValues();
                values.put(COLUMN_POSITION, maxPosition);
                values.put(COLUMN_PATH, song.getPath());
                values.put(COLUMN_TITLE, song.getTitle());
                values.put(COLUMN_ARTIST, song.getArtist());
                values.put(COLUMN_ALBUM, song.getAlbum());
